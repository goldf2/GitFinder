...
| 编号 | 优先级 | 状态 | 负责人 | 任务 |
| --- | --- | --- | --- | --- |
| GF-UPDATE-003 | P0 | 开发中 | Codex | 更新渠道、手动安装默认值与用户反馈 |
...
### GF-UPDATE-003 · 更新渠道、手动安装默认值与用户反馈

阶段：GF-P3；优先级：P0；状态：**开发中**；负责人：Codex；用户验收：待用户反馈。

更新：2026-09-21T05:15:32+08:00。依赖：无。

**验收标准**
1. 默认接收正式和测试版，保留已有退出测试版偏好。
...
更新：2026-09-21T04:02:02+08:00。依赖：无。

**验收标准**
1. electron-builder生成制品与元数据，electron-updater读取GitHub源。
2. 保留签名门禁，构建及本机检查验证；正式升级单独验收。

**接续动作：** GitHub alpha209测试包和原生更新清单已公开；恢复官网管理连接同步制品，配置Developer ID并完成正式自动安装验收。
...
证据：[docs/00-handoff/MATURE_UPDATES_ALPHA208.md](MATURE_UPDATES_ALPHA208.md)；[docs/00-handoff/RELEASE_ALPHA209_PUBLIC.md](RELEASE_ALPHA209_PUBLIC.md)

**阻塞/暂缓原因：** 官网浏览器连接失败且无发布凭据；有效Developer ID为0，正式签名/公证和真实旧版自动安装未验收。

### GF-PANEL-MANAGE-001 · 应用面板Coolify管理入口

阶段：GF-P1；优先级：P1；状态：**已交付**；负责人：Codex；用户验收：待用户反馈。

更新：2026-09-20T10:22:00+08:00。依赖：无。

**验收标准**
1. 卡片和表格均可进入对应Coolify部署管理页，缺少地址时禁用。
2. 没有公开域名也可管理，不探测远端或执行部署操作；完成安装验收。

**接续动作：** alpha205安装及原面板入口可见通过，源码已推送；浏览器回读超时，真实页面落地待用户确认。
...
交付：2.0.0-alpha.205；源码 `24e517dbab2567fbf5a38afbc352b702ea211a7b`。
...
更新：2026-09-16T21:30:11+08:00。依赖：GF-PROGRESS-001。
...
**接续动作：** 默认保持关闭；从设置测试区手动启用。后续GF-AI-PROGRESS-001先设计AI建议、证据与人工确认，不立即恢复默认开放或上传资料。
...
交付：2.0.0-alpha.196；源码 `f79dba1d2adf23f99adf5ff66d3667396552fe6a`。
...
更新：2026-09-19T06:40:07+08:00。依赖：GF-PANEL-NAME-001。
...
**接续动作：** 已完成源码、安装、重启、数据保护和推送；等待用户字号体验确认。默认值不变，旧版本的字号上限不能保证保留新大字号。
...
更新：2026-09-19T08:20:33+08:00。依赖：GF-TEXT-RANGE-001。
...
**接续动作：** 源码、安装38项与相邻55项回归、真实保存/冷启动、正常启动和推送已完成；等待用户实际反馈，明确确认后归档支持报告。
...
交付：2.0.0-alpha.202；源码 `a12551f5dd71702c2d1674e7f79b9b5130f17487`。
