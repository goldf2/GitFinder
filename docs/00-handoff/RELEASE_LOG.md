...
- 验证：关系白板专项 96/96，全量 `npm run check` 1091/1091，252 个 JavaScript 文件语法检查通过；已安装 `/Applications/GitFinder 2 Alpha130.app`，现场截图确认主界面只显示绿色同步状态灯，未显示资源统计文字；提交和推送待完成。
- 制品：`/Volumes/project/项目/gitfinder-2/dist/GitFinder-2-2.0.0-alpha.130-arm64-mac.zip`，SHA-256 `a1cba9a5f33005f077b0522badd7b242c12f2700eb8b8cf9f81041d677211be1`。
...
- 制品：`/Volumes/project/项目/gitfinder-2/dist/GitFinder-2-2.0.0-alpha.133-arm64-mac.zip`，SHA-256 `7ea74d9668131d298ad748572788ee16cdf55817d97ceef5fc226fd7e7326646`；`latest-mac.yml` 与 `release-verification.json` 同步生成，报告绑定提交 `4b1631c`。
...
- 提交：`53fc107`（功能）和 `4b1631c`（发布记录）已推送 `origin/main`；开发包为 ad-hoc 签名，不具备正式分发资格。
...
## 2026-09-19T06:40:07+08:00 · alpha.201

运行源码 `7ae44cdd3cbca46bc3ee01322889a5d64be6a1ed` 已安装验收并推送；1381测试/295JS与55安装交互通过，3原白板字节一致。ad-hoc开发签名；详细证据见[字号记录](TEXT_RANGES_ALPHA201.md)。

## 2026-09-19T08:20:33+08:00 · 2.0.0-alpha.202

容器四边四角缩放与自适应换行。运行源码`a12551f5dd71702c2d1674e7f79b9b5130f17487`，干净1399测试/297JS、18专项，安装93项通过且正常启动。原3白板与10项未提交内容保留。旧alpha201备份可恢复；只作本机开发签名，无公开发行。详见[容器缩放记录](CONTAINER_RESIZE_ALPHA202.md)。

## 2026-09-19T09:58:24+08:00 · 2.0.0-alpha.203

嵌套主机/Project标题安全区。源码`3dd763907c48bd88b6598bb2940989397d0bee19`已安装验收、正常启动并推送；精确1412/298JS、13专项，完整安装121项通过。3原白板与10项dirty保留；完整alpha202备份可恢复，仅macOS开发签名，无公开发行。详见[标题防重叠记录](CONTAINER_HEADER_ALPHA203.md)。
