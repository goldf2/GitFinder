...
**当前状态：alpha.199 已完成最终源码、安装与正常启动验证，源码已推送。** 以下开发中结果保留为历史，最终数量和制品以末节为准。
...
## 交付复核：隐藏整个图层也不能删掉已保存内容

首候选`af6cbb8`已完成开发包与安装版29+12+18流程，但未推送。复核相同保存入口时，又通过2项失败回归确认：`_combinedPlacements()`排除关闭的运行层/代码层，直接用它保存独立文档会漏掉已有资源及关系。修正为保留原显示投影，同时仅补回当前白板已持久化、因图层关闭而不显示的成员；不会导入未添加的实时资源，也不会用当前来源覆盖已保存的名称/说明。

最终45项专项（原29+新增16）通过；扩展源码UI32项通过，增加关闭运行层、真实文件读回、冷启动、再次显示恢复。架构层的已保存/未添加区分另有单测。首候选测试和元数据保留为initial文件，须从纠正提交完整重跑并重新构建安装，不能把首候选当最终交付。

## 最终交付 · 2026-09-18T23:09:43+08:00

- 运行源码 `5f7cbcca576824cc7a7673e21d3128d8d4391f89`（alpha.199），从干净工作树精确执行 `npm run check`：**1361/1361** 测试、**293** 个JS检查通过；资源/生命周期专项45/45（原29+新增16）。源码后续记录提交不改变安装包绑定来源。
- 最终安装版 **32/32** 容器/隐藏/显示层流程，**12/12** Coolify管理入口，**18/18** 实验功能门禁，共 **62项** 通过、无renderer异常。包括真实文件读回、多个进程重启、离线恢复、键盘及指针；架构隐藏层与不覆盖快照说明另有单测。
- 开发产物门禁issues为空，codesign深度严格校验通过，安装ASAR与构建报告一致：`177335ec6fef32b5e1cec2cbbd9c93a77c4349d90cb55ab4aa4d60c2e9d9acbd`。ZIP SHA-256：`40cc1bd55a92dc5a69468672cf81e89853e36143a0d7e9447d0156a7b691bb1e`。
- 安装：`/Applications/GitFinder 2.app`。旧alpha198恢复备份：`/Volumes/project/制品与备份/GitFinder-2-alpha198-before199.app.disabled`；本轮首候选另留存为`GitFinder-2-alpha199-first-candidate.app.disabled`，不作为最终发布源。
- 正常用户实例已启动，系统截图 `normal-final-alpha199.png` 确认alpha199、原项目、主机/Project外框、部署卡片与Coolify按钮。前两张截图仍在初始化/加载来源，未当作最终成功；未为截图移动或适配真实用户画布。
- 验收时两套原白板合集和独立con01白板共**3文件字节SHA-256与本轮安装前一致**。主目录原10项未提交内容保留，合并时仅恢复原差异；未自动补全、删除、整理实际白板。
- 运行源码在上述验收后推送到origin/main；记录收口及主工作树整合另核对最终SHA。证据汇总：`dist/agentdock-container-lifecycle-20260918/delivery-verification.json`；精确门禁`exact-source-check.log`，安装`installed-lifecycle-final.log`、`installed-links-final.log`、`installed-testing-final.log`。初候选证据均保留为initial，不混算结果。
- **限定**：macOS本机ad-hoc开发签名，未经Developer ID公证，不是公开Release；无Windows实机验收。UI采用合成来源和隔离profile，不替代真实Coolify网络/账号测试，不自动打开系统浏览器或登录后台。原有默认浏览器委托及精确URL校验未修改，实验进度页仍默认关闭。用户实际体验确认pending。

唯一下一任务恢复为GF-AI-PROGRESS-001（planned/unassigned，仅设计）；规模矩阵仍待后续优先级安排，不擅自开启AI上传、数据库迁移或服务器写操作。
