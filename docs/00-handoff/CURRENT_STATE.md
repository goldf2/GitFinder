...
## 当前交付：本机工作区资源组合与 Coolify 同步稳态（alpha.108，待打包）
...
- 当前实测：`coolify-topology-cache.json` 最近一次刷新为 `ready`，3 个配置实例、35 个部署、`errors=[]`；此前失败证据为 AL02 的 TCP 443 `ETIMEDOUT`，Con01 / AL03 可达。若再次失败，优先检查 AL02 到 `cool-al02` 的网络 / 反向代理 / TLS，而不是清空本地白板。
...
- 发布：版本已升至 `2.0.0-alpha.108`；macOS arm64 打包、本机替换、提交和推送尚未完成，完成后补充制品哈希和安装版证据。
