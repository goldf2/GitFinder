...
## 2026-09-15 · alpha.189 右侧面板调宽（源码与隔离交互通过，安装待验）

- 关系白板右侧停靠栏新增 6px 分隔线；支持指针拖拽、左右键微调、Shift 加速和双击恢复默认。
- 默认 264px，通常限制 220–640px；随窗口可用空间临时收窄并为画布保留空间，窗口恢复时回到用户偏好。
- 宽度单独保存在本机 `relationshipRightPanelWidth`，不改普通文件详情栏、白板数据或服务器。重绘/全屏/面板移出移回保持；拖拽取消、失焦与卸载清理已覆盖。
- 当前源码 1202/1202 测试、273 JS 检查通过；实际 Electron + 真实配置 IPC 的 15 项交互通过。安装/重启证据待补，见 [RIGHT_PANEL_WIDTH_ALPHA189.md](RIGHT_PANEL_WIDTH_ALPHA189.md)。
