...
## 当前修复：Coolify 同步日志与读取数据明细（alpha.113，已构建并替换本机）
...
- 界面：关系白板工具栏新增“日志”，直接显示最近同步、实例结果、已读取数量、端点成功 / 失败次数、耗时和逐事件时间线；可在文件管理器中定位日志文件。同步进行中和应用重启中断也会显示，不把部署失败与同步失败混为一谈。
...
- 发布：版本 `2.0.0-alpha.113`，代码提交 `a570febc66efe181da4a77ac497f9feafefa1f39` 已构建；macOS arm64 development ZIP `/Volumes/project/制品与备份/gitfinder-2/2.0.0-alpha.113/GitFinder-2-2.0.0-alpha.113-arm64-mac.zip`，SHA-256 `e43ea47cd9a0f671288afcb031938d70c30b829692ec7725baf5701a21b1be45`，ASAR SHA-256 `dae29cd1145aacd4013a79861fad72e59ff0dfc386939a4b3404905488d7fc8f`。
- 安装：已替换 `/Applications/GitFinder 2 Alpha87.app`，包内版本 alpha.113，codesign deep/strict 通过；替换前包备份于 `/Volumes/project/制品与备份/gitfinder-2/installed-backups/alpha113-pre-final-rebuild-20260907-062833/`，alpha.112 仍可从上一份备份恢复。开发包为 ad-hoc 签名，尚未上传商店或切换公开 current。
- 待用户验收：旧 alpha.112 尚未生成日志；新版首次同步后点击白板工具栏“日志”，确认真实端点和读取计数显示。为遵守当前访问边界，本轮没有额外探测 AL02。
