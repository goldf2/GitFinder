...
<!-- BEGIN CURRENT_STATE -->
## 当前有效快照 · alpha.198 Coolify卡片入口已交付

更新时间：2026-09-18T09:43:56+08:00。GF-COOLIFY-LINK-001已交付；下一任务GF-AI-PROGRESS-001仍planned/unassigned、仅设计。

- 当前安装alpha198，运行源码`e9097dce888cf2511d68bfcccc3b432290d1cdf8`，新增默认浏览器QA提交`ba0f6c5abbacd7a318c69b43273e06ffd2f72b84`，已在安装验收后推送；文档收口不改变制品源。
- 部署卡片、主机与云Project标题有直接Coolify入口；经来源地址和主进程精确白名单校验后调用系统默认浏览器，不指定Chrome/Safari，不用内嵌后台。网站访问保持独立，缺来源时禁用说明，多实例不串源。
- 最终1345/1345测试、293JS；16项专项。安装版12项链接+21项容器+18项测试区=51项通过，含重启/离线恢复；安装ASAR主进程默认浏览器委托另经VM边界验证。未代用户登录后台。
- 正常用户截图显示alpha198、原项目和实际卡片按钮；首次初始化截图不当作完成。原10项dirty保留；2个白板文件字节一致，当前工作区在新包生成前已有一次写入差异，未覆盖、不宣称全部字节不变。
- 旧197备份保留；仅macOS ad-hoc开发包。实验进度仍默认关闭，没有调用AI、迁移数据库、合入双源更新或写入Coolify。

详情、校验限定与接续见[COOLIFY_CARD_LINKS_ALPHA198.md](COOLIFY_CARD_LINKS_ALPHA198.md)。
<!-- END CURRENT_STATE -->
