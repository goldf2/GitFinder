...
<!-- BEGIN CURRENT_STATE -->
## 当前有效快照 · alpha.196 测试功能区已交付

更新时间：2026-09-16T21:30:11+08:00。GF-LABS-001已交付；唯一下一任务：GF-AI-PROGRESS-001（仅设计，未认领）。

- 当前安装alpha.196，运行源码`f79dba1d2adf23f99adf5ff66d3667396552fe6a`已在安装验收后推送，记录收口不改变制品源。
- 设置 → 测试功能提供仪表盘、开发进度（开发任务）两个独立本机开关，默认关闭。菜单、旧标签/分离窗口、主进程入口与刷新遵守开关，禁用不删除台账/白板。
- 精确源码1300/1300测试、288JS；新增16项专项。最终安装版18项设置门禁（含跨窗口及两次重启）+9项显式开启后进度流程通过，系统截图确认正常用户启动/旧任务页回到文件浏览。
- 两套白板字节哈希与安装前一致，主目录原10项dirty按基线保留。旧195备份在`/Volumes/project/制品与备份/GitFinder-2-alpha195-before196.app.disabled`。
- AI辅助进度仍为待设计方向，不自动判定完成、回写状态、运行开发或上传源码/台账。仓库内management和接续文档照常维护，不因界面隐藏而停用。
- 初轮窄窗说明拥挤已修；额外轮询测试安装版失败诊断为窗口hidden，按测试前提补可见性模拟，没有移除产品隐藏暂停规则。更早一次源码超时无诊断，原因不追认。全部失败记录保留。
- 仅macOS ad-hoc开发包，Windows、公证、公开发行未执行；CDP输入不冒充原生AX点击，用户反馈未代签。

详情与复跑见[EXPERIMENTAL_FEATURES_ALPHA196.md](EXPERIMENTAL_FEATURES_ALPHA196.md)。任务事实仍在[原台账](../../management/development-tasks.json)，阅读看板[PROGRESS.md](PROGRESS.md)。
<!-- END CURRENT_STATE -->
