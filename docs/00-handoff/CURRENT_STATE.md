...
<!-- BEGIN CURRENT_STATE -->
## 当前有效快照 · alpha.202 容器缩放已交付

更新时间：2026-09-19T08:20:33+08:00。GF-CONTAINER-RESIZE-001已交付；唯一下一任务GF-AI-PROGRESS-001仍planned/unassigned、仅设计。

- 当前安装alpha202，运行源码`a12551f5dd71702c2d1674e7f79b9b5130f17487`已在完整安装验收后推送；记录收口不改变制品来源。
- 主机、Project、手工群组支持四边四角拖动，按宽度换行且内部卡片/文字不缩放。嵌套容器整体排列，尺寸不足时保持内容最小空间。
- 预览不提交半次几何；一次结束对应一次撤销。Esc、失焦、来源刷新/文档切换取消。锁定容器或相关成员禁止重排；保存文件与动态配置都保留显式宽高和锚点。
- 干净源码1399/1399、297JS、18专项；安装38缩放+11字号+32资源+12Coolify=93项通过。正常用户系统截图确认原项目及白板恢复。
- 原10项dirty和3原白板保留；alpha201备份可恢复。新操作保存可选wrap字段，旧严格解析版本可能不识别新增字段，应使用alpha202及兼容后续版本打开新缩放文件。
- 仅macOS开发签名，无Windows/公证/公开发行，无远端配置写入、数据库迁移或AI调用；实验进度默认关闭不变。

详细证据：[容器缩放](CONTAINER_RESIZE_ALPHA202.md)。
<!-- END CURRENT_STATE -->
