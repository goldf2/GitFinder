...
<!-- BEGIN CURRENT_STATE -->
## 当前有效快照 · alpha.203 标题防重叠已交付

更新时间：2026-09-19T09:58:24+08:00。GF-CONTAINER-HEADER-001已交付；唯一下一开发任务GF-AI-PROGRESS-001仍planned/unassigned，仅设计，不自动接入模型。

- 当前安装alpha203，运行源码`3dd763907c48bd88b6598bb2940989397d0bee19`已在完整安装验收及正常启动后推送；最终记录提交不改变制品来源。
- 主机、Project及内嵌群组从内到外保留独立标题区，修复显式wrap同顶边覆盖。拖动/边角缩放沿用标题安全区，中等及大字号一并处理，嵌套标题使用容器可用宽度。
- 打开旧重叠布局时只补必要显示外框，内部卡片世界坐标与关系保留；不擅自改写原文件。不隐藏标题/Coolify入口，也不改变字体默认值。
- 确切源码1412/1412、298JS、13专项；安装28标题+38缩放+11字号+32资源+12Coolify=121项完整通过，含真实文件/配置写入、冷启动、拖动预览与撤销重做。
- 正常无调试参数启动，系统截图确认原项目/应用面板及con01白板，父子标题分层。3原白板在正常启动后与安装前基线字节一致；原10项dirty保留，合入仅更新本轮路径。
- alpha202完整备份可恢复；仅macOS ad-hoc开发包，无Windows/公证/公开发行。实验仪表盘及开发进度仍默认关闭，无数据库迁移、远端配置写入或AI调用。

证据：[标题防重叠](CONTAINER_HEADER_ALPHA203.md)。
<!-- END CURRENT_STATE -->
