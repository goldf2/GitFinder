...
<!-- BEGIN CURRENT_STATE -->
## 当前有效快照 · alpha.199 容器生命周期已交付

更新时间：2026-09-18T23:09:43+08:00。GF-CONTAINER-LIFECYCLE-001已交付；唯一下一任务GF-AI-PROGRESS-001仍planned/unassigned、仅设计。

- 当前安装alpha199，运行源码`5f7cbcca576824cc7a7673e21d3128d8d4391f89`已在实际安装验收后推送，记录收口不改变制品来源。
- 在线/离线Project使用稳定身份：标题、右键和Delete一致为隐藏，成员不因runtime缺失而可随意移组或被手动加入，普通手工群组仍可解散。
- 独立文档保存/另存为/导出保留隐藏列表；关闭运行或架构层不再丢弃已保存成员/关系，也不导入未添加实时资源，不覆盖原快照名称说明。
- 最终精确1361/1361、293JS，45专项（新16）；安装32生命周期+12Coolify入口+18实验门禁=62项通过。真实磁盘/进程重启与离线恢复已验证，正常用户截图显示原项目/卡片/管理入口。
- 3个原白板文件验收时字节一致；原10项dirty保留。198备份可恢复；本轮首候选后经补充覆盖纠正，不混用初候选结果。
- Coolify入口继续交给系统默认浏览器，实验进度默认关闭；无真实后台登录、服务器写入、AI调用、数据库迁移或双源更新合并。仅macOS开发签名，Windows/公证/公开发行未执行。

详细证据和失败边界：[CONTAINER_LIFECYCLE_ALPHA199.md](CONTAINER_LIFECYCLE_ALPHA199.md)。
<!-- END CURRENT_STATE -->
