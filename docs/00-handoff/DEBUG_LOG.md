...
## GF-DIR-20260909-01 · 点击项目目录后侧栏排序跳动

- 时间：2026-09-09 09:10:00 +0800；状态：alpha.133 已修复、打包并安装，待用户确认。
- 现象：点击“项目”侧栏中的目录后，当前项目被移动到“最近”列表首位，列表整体跳动。
- 证据：录屏 `/var/folders/x6/p67_lpd91j71s72dhzg1vkzh0000gn/T/TemporaryItems/NSIRD_screencaptureui_v5F6gd/录屏2026-09-09 08.49.35.mov`；连续点击多个项目时“最近”首项依次变化。
- 根因：目录导航调用 `recordProjectVisit()`；`ProjectShortcutsController.recordVisit()` 更新时间后立即 `render()`，`resolveDisplay()` 按 `lastOpenedAt` 排序，造成 DOM 顺序变化。
- 修复：访问时间和稳定 ID 仍写入 `projectShortcuts`，导航时改为 `syncActiveState()`，不重绘整个快捷列表；显式刷新、设置变更或重启仍会按保存记录重新排序。
- 回归：`test/project-shortcuts-controller.test.js` 新增“不重绘项目快捷列表”测试；`npm run check` 通过 1100/1100，252 个 JavaScript 文件语法检查通过。
- 发布：版本 `2.0.0-alpha.133`；ZIP SHA-256 `7ea74d9668131d298ad748572788ee16cdf55817d97ceef5fc226fd7e7326646`；安装 `/Applications/GitFinder 2 Alpha133.app`，签名校验通过，发布报告绑定提交 `4b1631c`。用户确认有效后归档支持文档修复报告。
...
# GF-20260913-01 主机容器显示设置菜单无效

- 现象：主机容器的“更多操作 → 显示设置…”可见，但点击后没有可靠地打开或应用主机的显示层级。
- 根因：主机容器是不可选中的 `hostBubble` 节点，显示设置动作只从当前选中卡片解析实体；白板刷新或选择同步后选中态可能为空。
- 修复：上下文菜单记录打开它的实体 ID；显示设置、重置和多选层级切换在选中态为空时回退到该实体。
- 验证：`node --test test/relationship-resource-display.test.js test/relationship-board-action-router.test.js` 20/20；`npm run check` 1163/1163；alpha.163 已安装。现场菜单回归待用户确认层级切换后的下级资源显示。
# GF-20260913-02 主机与 Project 容器快捷菜单不一致

- 现象：主机容器使用白板右键菜单，Project 容器使用浮动工具条，两者入口和操作不一致。
- 修复：主机改为复用 Project 容器的 React Flow `NodeToolbar` 与 `⋯` 展开状态；统一提供显示设置、自动排列与固定下级/解除固定，主机不显示不适用的编辑/解散操作。
- 额外修复：主机气泡补充真实 placement，保证固定下级按钮能写入对应主机布局；主机自动排列过滤普通本地项目，只整理本主机的 Coolify Project/未分类部署。
- 验证：专项测试与完整 `npm run check` 通过（1165/1165，270 个 JS）；alpha.165 已打包并安装。CUA native pipe 超时，安装版菜单外观仍需用户现场确认。

# GF-20260913-03 主机按钮再次无法打开

- 现象：安装 alpha.165 后，主机容器标题或 `⋯` 按钮点击无响应，主机快捷菜单无法稳定展开。
- 根因：主机按钮没有完整复用普通卡片的 pointer/mouse 按下隔离与 `.gf-flow-more` 菜单标识；白板的全局 pointerdown 关闭逻辑可能先把主机按钮识别为画布外部点击。
- 修复：主机标题和 `⋯` 按钮均停止 pointer/mouse 按下冒泡；`⋯` 增加 `gf-flow-more`、`aria-expanded`，并保留点击状态切换。
- 验证：白板 UI、Flow Adapter、资源显示专项与完整 `npm run check` 通过（1165/1165，270 个 JS）；alpha.166 已打包并安装。CUA native pipe 仍超时，现场点击回归待用户确认。

# GF-20260913-05 标题体系不一致及访问点显示方向修正

- 时间：2026-09-13 +0800；用户连续反馈 alpha.167 仍异常，并提供主机标题巨大、Project 标题很小的截图。
- 当前证据：原生 App 可读取；旧主机标题在画布内随世界坐标缩放，Project 标题使用 NodeToolbar；主机工具条在整个容器底部展开而非标题旁。旧“独立固定节点”把访问点留在远端，适合全部时图形过小。
- 修复：ContainerHeader 共用缩放、顶部左对齐和菜单；访问点作为部署内固定子内容；无连线模式布局排除远端端点坐标，计入子内容高度。自动成员按稳定身份排序避免高度居中后排序翻转。挂载前视口请求延迟到节点测量完成。
- 回归：1170 项测试通过、270 个 JS 语法检查通过；隔离浏览器点击两种容器菜单和访问点子内容通过。安装和用户确认待完成；此前 GF-20260913-03 的事件隔离只修正部分路径，不是完整根因。

# GF-20260913-04 关系连线遮挡访问点与容器操作（方案已被 GF-20260913-05 取代）

- 现象：关系白板中的长连线、缩放和主机容器菜单交互相互干扰；用户要求访问点不依赖关系线显示。
- 修复：新增白板 `showRelationshipLines` 显示偏好，默认关闭；关闭时适配器过滤渲染边，并将访问点从 Project 父级拆为独立固定节点，仅改变显示投影，不写回原始关系归属。运行拓扑菜单可重新开启连线。
- 验证：Flow Adapter、Action Router、UI、Graph Model 专项及完整检查通过（1169/1169，270 个 JS）；alpha.167 已打包并安装。CUA native pipe 超时，安装版视觉确认待用户确认。
