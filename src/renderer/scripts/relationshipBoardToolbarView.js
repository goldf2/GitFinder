(function exposeRelationshipBoardToolbarView(root, factory) {
  ...
  function displayPopover({ view, boardView, serverTree, icon, cardIconOptions = [], escapeHtml: escape }) {
    ...
    const zoomControls = select('edgeZoomMode', '连线随视图缩放', view.edgeZoomMode || 'adaptive', [['adaptive', '有限补偿（缩小后变细、变淡）'], ['follow', '完全跟随卡片缩放'], ['fixed', '固定屏幕粗细']], escape)
      + select('maxZoom', '最大放大倍数', String(view.maxZoom || 8), [['1', '1 倍（100%）'], ['2', '2 倍（200%）'], ['4', '4 倍（400%）'], ['8', '8 倍（800%）']], escape);
    return `<div class="relationship-display-host">
      ...
      <div class="relationship-display-popover" role="dialog" aria-modal="true" aria-labelledby="relationship-display-title" hidden tabindex="-1"><form data-relationship-display-form>
        ...
        <div class="relationship-display-sections">
          <section class="relationship-display-section"><header><h3>卡片与文字</h3><p>控制信息密度、尺寸和文字层级</p></header>${select('mode', '信息密度', view.mode, [['full', '完整'], ['compact', '精简']], escape)}${sliders.slice(0, 8).map(item => slider(item, escape)).join('')}</section>
          <section class="relationship-display-section"><header><h3>布局与间距</h3><p>自动排列和容器边界使用同一组间距</p></header>${sliders.slice(8, 10).map(item => slider(item, escape)).join('')}${select('projectGroupShape', 'Project 容器形状', view.projectGroupShape, [['rounded', '矩形'], ['polygon', '多边形']], escape)}</section>
          ...
          <section class="relationship-display-section"><header><h3>状态与外观</h3><p>调整卡片层次、状态色和筛选强调</p></header>${select('cardAppearance', '卡片层次', view.cardAppearance, [['elevated', '层次阴影'], ['flat', '简洁平面']], escape)}${sliders.slice(10).map(item => slider(item, escape)).join('')}</section>
          <section class="relationship-display-section relationship-display-section-wide"><header><h3>画布与关系</h3><p>这些选项只改变呈现，不修改关系事实</p></header>${slider(edgeWidthSlider, escape)}${zoomControls}<div class="relationship-display-toggles">
