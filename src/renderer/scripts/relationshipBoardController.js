(function exposeRelationshipBoardController(root, factory) {
    ...
    for (const [boardId, placements] of boardEntries) {
      ...
      for (const [entityId, placement] of Object.entries(placements).slice(0, 2000)) {
        ...
        boardLayout[entityId] = {
          ...
          ...Object.fromEntries(['groupBackground', 'groupBorder'].filter(key => /^#[0-9a-f]{6}$/i.test(placement[key] || '')).map(key => [key, placement[key]])),
  ...
  class Controller {
    ...
    async _setLayer(layer) {
      // Compatibility route for old saved toolbar actions. New UI controls
      // each source independently and never treats them as mutually exclusive.
      const next = String(layer || 'runtime');
      if (!Model.BOARD_LAYERS.includes(next)) return false;
      return next === 'architecture'
        ? this._setArchitectureVisibility(true)
        : this._setTopologyVisibility(true);
    }

    _setTopologyVisibility(show) {
      const board = activeBoard(this.store);
      if (!board) return false;
      const next = Boolean(show);
      if (this._boardView().showTopology === next) return false;
      board.view = { ...this._boardView(), showTopology: next };
      this.resourcePreview = null;
    ...
    async _setArchitectureVisibility(show) {
      ...
      if (!board) return false;
      const next = Boolean(show);
      if (next) await this._refreshArchitectureSnapshots({ render: false });
      if (this._boardView().showArchitecture === next) return false;
      board.view = { ...this._boardView(), showArchitecture: next };
      this.resourcePreview = null;
      this._clearEntitySelection();
      this.selectedRelationshipId = '';
      this._persistSoon(0);
      this.render();
      if (next && !this.architectureProjection.entities.length) {
    ...
    _resourceCatalog() {
      ...
      const board = activeBoard(this.store);
      const hidden = this._hiddenResourceIds();
      const boardPlacements = Array.isArray(board?.placements) ? board.placements.filter(item => !hidden.has(item.entityId)) : [];
    ...
    _applyResourceDisplayPreferences(topologyPlacements, visibleIds, relationships) {
      ...
      for (const [rootId] of preferences) {
        ...
        if (entity.type === 'server') {
          ...
          // Descendant visibility belongs to the host, including unchecked
          // levels. Keep structural parents when an endpoint is requested.
          if (levels.includes('endpoint') && !levels.includes('deployment')) levels.push('deployment');
          if (levels.includes('deployment') && !levels.includes('project')) levels.push('project');
          ...
          const levels = preferences.get(rootId) || [];
          const deploymentIds = new Set(hierarchy.deploymentsByProject.get(rootId) || []);
          for (const placement of topologyPlacements) if (placement.groupId === rootId && entities.get(placement.entityId)?.type === 'deployment') deploymentIds.add(placement.entityId);
          for (const deploymentId of deploymentIds) {
            if (levels.includes('deployment') || levels.includes('endpoint')) require(deploymentId, rootId, 'deployment');
            if (levels.includes('endpoint')) for (const endpointId of hierarchy.endpointsByDeployment.get(deploymentId) || []) require(endpointId, rootId, 'endpoint');
          }
        } else if (entity.type === 'deployment') {
          if (preferences.get(rootId)?.includes('endpoint')) for (const endpointId of hierarchy.endpointsByDeployment.get(rootId) || []) require(endpointId, rootId, 'endpoint');
        }
      }
      const next = new Set(visibleIds);
      for (const [entityId, entries] of requirements) {
        // A host's Project switch is the outer boundary. A Project's own
        // preference must not resurrect it after its host excluded it.
        const hostEntries = entries.filter(entry => entities.get(entry.rootId)?.type === 'server');
        if (hostEntries.length) {
          if (hostEntries.every(entry => !preferences.get(entry.rootId)?.includes(entry.threshold))) next.delete(entityId);
          continue;
        }
    ...
    _topologyVisible() {
      return this._readBoardView().showTopology !== false;
    }

    _architectureVisible() {
      return this._readBoardView().showArchitecture === true;
    }

    _readBoardView() {
      const board = activeBoard(this.store);
      if (!board) return Model.defaultBoardView();
      const raw = board.view || {};
      const rawLayer = String(raw.layer || 'runtime');
      const view = { ...Model.defaultBoardView(), ...raw, ...Model.boardOrganization(raw) };
      if (typeof raw.showTopology !== 'boolean') view.showTopology = rawLayer !== 'architecture';
      if (typeof raw.showArchitecture !== 'boolean') view.showArchitecture = rawLayer === 'architecture' || rawLayer === 'merged';
      if (view.layer === 'merged') view.layer = 'runtime';
      delete view.topologyLayout;
      delete view.treeLayout;
      return view;
    }

    _isArchitectureEntity(entity, placement = null) {
      return entity?.type === 'architecture'
        || Boolean(entity?.details?.architectureSnapshotId)
        || this.architectureProjection?.placements?.some(item => item.entityId === (placement?.entityId || entity?.id))
        || placement?.architectureReadOnly === true;
    }
    ...
    _combinedEntities() {
      const topologyVisible = this._topologyVisible();
      ...
      const entities = [];
      const add = entity => {
        if (!entity || aliases.has(entity.id)) return;
        const architecture = this._isArchitectureEntity(entity);
        const topology = this._isTopologyEntity(entity);
        if ((architecture && !architectureVisible) || (topology && !topologyVisible)) return;
        const liveEntity = live.get(entity.id);
        const value = liveEntity ? { ...entity, runtime: liveEntity.runtime } : entity;
        if (!entities.some(item => item.id === value.id)) entities.push(value);
      };
      for (const entity of this.store?.entities || []) add(entity);
      if (topologyVisible) for (const entity of this.panelProjection?.entities || []) add(entity);
      if (architectureVisible) for (const entity of this.architectureProjection?.entities || []) add(entity);
      return entities;
    }
    ...
    _combinedPlacements(board = activeBoard(this.store)) {
      ...
      if (architectureVisible) for (const placement of this.architectureProjection?.placements || []) {
        if (!ids.has(placement.entityId) && entities.has(placement.entityId)) {
          ids.add(placement.entityId);
          placements.push(placement);
        }
      }
    ...
    _unarchivedPlacements() {
      const placements = this._combinedPlacements();
      const hidden = this._hiddenResourceIds();
      for (const item of placements) if (item.archived) hidden.add(item.entityId);
    ...
    _hiddenResourceIds() {
      const hidden = new Set(activeBoard(this.store)?.hiddenResourceIds || []);
      if (!hidden.size) return hidden;
      const placements = this._combinedPlacements();
      const hierarchy = this._resourceHierarchy([...this._allEntitiesById().values()], this._combinedRelationships(placements));
      for (const id of [...hidden]) {
        for (const project of hierarchy.projectsByServer.get(id) || []) hidden.add(project);
        for (const deployment of hierarchy.deploymentsByServer.get(id) || []) hidden.add(deployment);
      }
      let changed = true;
      while (changed) {
        changed = false;
        for (const item of placements) if (hidden.has(item.groupId) && !hidden.has(item.entityId)) { hidden.add(item.entityId); changed = true; }
      }
      const endpointOwners = new Map();
      for (const [deployment, endpoints] of hierarchy.endpointsByDeployment) for (const endpoint of endpoints) {
        if (!endpointOwners.has(endpoint)) endpointOwners.set(endpoint, []);
        endpointOwners.get(endpoint).push(deployment);
      }
      for (const [endpoint, owners] of endpointOwners) if (owners.every(id => hidden.has(id))) hidden.add(endpoint);
      return hidden;
    }

    _scopePlacements(placements = this._unarchivedPlacements()) {
      const view = this._readBoardView();
      const entities = this._allEntitiesById();
      const architecturePlacements = placements.filter(item => this._isArchitectureEntity(entities.get(item.entityId), item));
      const topologyPlacements = placements.filter(item => this._isTopologyEntity(entities.get(item.entityId)));
      const canvasPlacements = placements.filter(item => (
        !this._isArchitectureEntity(entities.get(item.entityId), item)
        && !this._isTopologyEntity(entities.get(item.entityId))
      ));

      const architectureMode = Model.ARCHITECTURE_SCOPE_MODES.includes(view.architectureScopeMode)
        ? view.architectureScopeMode : 'snapshot';
      let architectureVisibleIds = new Set(architecturePlacements.map(item => item.entityId));
      if (architectureMode === 'boundary') {
        architectureVisibleIds = new Set([String(view.architectureScopeId || '')].filter(Boolean));
        let changed = true;
        while (changed) {
          changed = false;
          for (const placement of architecturePlacements) {
            if (placement.groupId && architectureVisibleIds.has(placement.groupId) && !architectureVisibleIds.has(placement.entityId)) {
              architectureVisibleIds.add(placement.entityId); changed = true;
            }
          }
        }
      } else if (architectureMode === 'component') {
        const rootId = String(view.architectureScopeId || '');
        architectureVisibleIds = new Set(rootId ? [rootId] : []);
        const selected = new Set([rootId]);
        for (const edge of this._combinedRelationships(architecturePlacements)) {
          if (selected.has(edge.sourceId)) architectureVisibleIds.add(edge.targetId);
          if (selected.has(edge.targetId)) architectureVisibleIds.add(edge.sourceId);
        }
        // Keep the boundary frame when one of its members is visible.
        for (const placement of architecturePlacements) {
          if (placement.groupId && architectureVisibleIds.has(placement.entityId)) architectureVisibleIds.add(placement.groupId);
        }
      }
      const showBoundaries = view.architectureShowBoundaries !== false;
      const visibleArchitecture = architecturePlacements
        .filter(item => architectureVisibleIds.has(item.entityId) && (showBoundaries || entities.get(item.entityId)?.type !== 'group'))
        .map(item => {
      ...
      if (topologyMode === 'board') {
        ...
        for (const edge of this._combinedRelationships(topologyPlacements)) {
          if (edge.sourceId === rootId) topologyVisibleIds.add(edge.targetId);
          if (edge.targetId === rootId) topologyVisibleIds.add(edge.sourceId);
        }
        let changed = true;
        while (changed) {
          changed = false;
          for (const placement of topologyPlacements) {
            if (placement.groupId && topologyVisibleIds.has(placement.groupId) && !topologyVisibleIds.has(placement.entityId)) {
              topologyVisibleIds.add(placement.entityId); changed = true;
            }
      ...
      const visibleIds = new Set([
        ...
        ...visibleTopology.map(item => item.entityId),
        ...visibleArchitecture.map(item => item.entityId)
      ]);
      return placements.filter(item => visibleIds.has(item.entityId)).map(item => (
        visibleArchitecture.some(candidate => candidate.entityId === item.entityId) ? visibleArchitecture.find(candidate => candidate.entityId === item.entityId) : item
      ));
    }
    ...
    _combinedRelationships(placements = this._combinedPlacements()) {
      ...
      const aliases = this._topologyVisible() ? this._endpointAliases() : new Map();
      const relationships = [];
      const facts = new Set();
      const relationshipSources = [
        this.store?.relationships || [],
        this._topologyVisible() ? this.panelProjection?.relationships || [] : [],
        this._architectureVisible() ? this.architectureProjection?.relationships || [] : []
      ];
    ...
    revealResource(kind, refId) {
      ...
      // Projects and repositories are runtime resources. Showing one must not
      // hide an already visible architecture snapshot on the same board.
    ...
    _arrangeHost(hostId) {
      ...
      // Live Coolify deployments can carry their Project container through
      // placement.groupId before the normalized hierarchy is rebuilt.
      const placementById = new Map(placements.map(item => [item.entityId, item]));
      for (const relationship of this._combinedRelationships(placements)) {
        if (relationship.type !== 'runs_on' || relationship.targetId !== hostId) continue;
        const deployment = entities.get(relationship.sourceId);
        const groupId = placementById.get(relationship.sourceId)?.groupId;
        if (deployment?.type === 'deployment') {
          deploymentIds.add(deployment.id);
          if (groupId && this._isProjectGroup(groupId)) projectIds.add(groupId);
        }
      }
      for (const placement of placements) {
        if (placement.groupId && this._isProjectGroup(placement.groupId)
          && deploymentIds.has(placement.entityId)) projectIds.add(placement.groupId);
      }
    ...
    _filteredGraph() {
      ...
      const entitiesById = this._allEntitiesById();
      const hidden = this._hiddenResourceIds();
      const placements = this._scopePlacements(this._unarchivedPlacements()).filter(item => !hidden.has(item.entityId));
      ...
      if (this._isServerTree()) {
        ...
        // Ownership must survive hiding all deployments of a Project.
        // Derive it from the source topology, not the filtered cards.
        const sourceHierarchy = this._resourceHierarchy([...entitiesById.values()], this._combinedRelationships(this._combinedPlacements()));
        const visible = new Set(tree.placements.map(item => item.entityId));
        for (const [hostId, projectIds] of sourceHierarchy.projectsByServer) {
          for (const projectId of projectIds) {
            if (!visible.has(hostId) || !visible.has(projectId)
              || tree.summaryRelationships.some(edge => edge.sourceId === hostId && edge.targetId === projectId)) continue;
            tree.summaryRelationships.push({ id: `tree_${hostId}:${projectId}`, type: 'tree_hierarchy', sourceId: hostId, targetId: projectId, label: 'Project' });
          }
        }
        return { ...graph, ...tree };
      }
    ...
    _contextMenuItems(kind) {
      ...
      if (kind === 'relationship') {
        ...
        const entity = this._allEntitiesById().get(this.contextMenuEntityId)
          || [...this._entitySelectionIds()].map(id => this._allEntitiesById().get(id)).find(Boolean);
        ...
        const projectRequired = (selectedLevels.has('deployment') || selectedLevels.has('endpoint')) && entity?.type === 'server';
        if (projectRequired) selectedLevels.add('project');
        const deploymentRequired = selectedLevels.has('endpoint') && ['server', 'project'].includes(resourceDisplayType(entity));
        if (deploymentRequired) selectedLevels.add('deployment');
        if (levels.length) items.push(context('显示内容（可多选）', 'resource-display-heading', true));
        items.push(...levels.map(([level, label]) => context(
          `${selectedLevels.has(level) ? '✓ ' : '○ '}${label}${level === 'project' && projectRequired ? '（包裹部署）' : ''}${level === 'deployment' && deploymentRequired ? '（包裹访问点）' : ''}`,
          `resource-display-toggle:${level}`,
          level === anchor || level === 'project' && projectRequired || level === 'deployment' && deploymentRequired,
    ...
    _runContextAction(action) {
      ...
      if (String(action || '').startsWith('resource-display-toggle:')) {
        ...
        const entity = (contextEntity && resourceDisplayOptions(contextEntity).length ? contextEntity : null)
          || [...this._entitySelectionIds()].map(id => this._allEntitiesById().get(id)).find(item => item && resourceDisplayOptions(item).length);
      ...
      if (action === 'resource-settings') {
        const entity = contextEntity || [...this._entitySelectionIds()].map(id => this._allEntitiesById().get(id)).find(Boolean);
      ...
      if (action === 'hide-resource') return this._hideResourceSelection();
    ...
    _setResourceDisplayLevels(levels = [], targetEntity = null) {
      const entity = (targetEntity && resourceDisplayOptions(targetEntity).length ? targetEntity : null)
        || [...this._entitySelectionIds()].map(id => this._allEntitiesById().get(id)).find(item => item && resourceDisplayOptions(item).length);
      const placement = entity ? this._placementForEntity(entity.id) : null;
      ...
      delete placement.resourceDisplayProjectEnabled;
    ...
    _toggleResourceDisplayLevel(level, targetEntity = null) {
      const entity = (targetEntity && resourceDisplayOptions(targetEntity).some(option => option[0] === level) ? targetEntity : null)
        || [...this._entitySelectionIds()].map(id => this._allEntitiesById().get(id)).find(item => item && resourceDisplayOptions(item).some(option => option[0] === level))
        || this._allEntitiesById().get(this.contextMenuEntityId);
      ...
      panel.innerHTML = `
        ...
        <div class="relationship-runtime-inspector">
          ...
          ${selected.kind === 'entity' ? `<form class="relationship-inspector-form relationship-annotation-form" data-relationship-annotation-form data-entity-id="${escapeHtml(fact.id)}">
            ${fact.type === 'group' ? this._groupAppearanceEditorHtml(fact.id) : ''}
    ...
    _addResource(resource, point = null) {
      ...
      if (resource.entityId) {
        const board = activeBoard(this.store);
        if (board?.hiddenResourceIds?.includes(resource.entityId)) {
          this._recordMutation();
          board.hiddenResourceIds = board.hiddenResourceIds.filter(id => id !== resource.entityId);
          this._finishBoardMutation();
          this._renderResources();
          return;
        }
    ...
    _ensureRuntimeLayerForResource(kind) {
      ...
      const board = activeBoard(this.store);
      if (!board || this._topologyVisible()) return false;
      board.view = { ...this._boardView(), showTopology: true };
      this._persistSoon(0);
      return true;
    }
    ...
    _hideResourceSelection() {
      const selectedIds = this._entitySelectionIds();
      if (!selectedIds.size) return false;
      const board = activeBoard(this.store);
      if (!board) return false;
      const hidden = new Set(selectedIds);
      let changed = true;
      while (changed) {
        changed = false;
        for (const placement of this._combinedPlacements(board)) {
          if (hidden.has(placement.groupId) && !hidden.has(placement.entityId)) { hidden.add(placement.entityId); changed = true; }
        }
      }
      this._recordMutation();
      board.hiddenResourceIds = [...new Set([...(board.hiddenResourceIds || []), ...selectedIds])];
      this._clearEntitySelection();
      this._finishBoardMutation();
      this._renderResources();
      this.notify(`已从当前白板隐藏 ${hidden.size} 个资源`, 'success');
      return true;
    }
