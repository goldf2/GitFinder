...
function createUpdateService({
  ...
  function status() {
    return {
      ...
      manualInstall: Boolean(autoUpdater?.manualInstall),
  ...
  async function downloadUpdate() {
    ...
    const options = {
      ...
      detail: autoUpdater.manualInstall ? '下载并验签后打开安装包位置；当前开发签名版本需手动安装。' : '下载完成后仍由你决定何时重启安装；取消不会修改当前版本。',
      buttons: ['下载更新', '取消'],
      defaultId: 0,
      cancelId: 1,
      noLink: true,
  ...
  function bindEvents() {
    ...
    autoUpdater.on('update-downloaded', () => {
      ...
      dialog.showMessageBox(window, {
        ...
        title: '更新已下载',
        message: '更新已下载完成',
        detail: autoUpdater.manualInstall ? '当前为开发签名，需手动替换应用程序。请先保存正在编辑的内容。' : '重启前请先保存正在编辑的内容。',
        buttons: [autoUpdater.manualInstall ? '打开安装包位置' : '立即重启', '稍后'],
        defaultId: 0,
        cancelId: 1,
      }).then(({ response }) => {
