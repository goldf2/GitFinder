...
# 输出:
#   dist/GitFinder 2 Alpha-darwin-arm64/GitFinder 2 Alpha.app  — 可运行应用
#   dist/GitFinder-2-{version}-arm64-mac.zip                    — Alpha 验证包
# ============================================================
set -e

APP_NAME="GitFinder 2 Alpha"
ARTIFACT_PREFIX="GitFinder-2"
...
ZIP_NAME="${ARTIFACT_PREFIX}-${VERSION}-${ARCH}-mac.zip"
...
# 打包
echo "[3/8] 使用 @electron/packager 打包并在签名前收紧 Fuses..."
node scripts/package-mac.js

# 正式模式已由 Packager 在打包阶段签名、公证和附加票据；开发模式保持 ad-hoc。
echo "[4/8] 完成签名策略..."
if [ "${RELEASE_MODE}" = "development" ]; then
  codesign --force --deep --sign - --identifier com.gitfinder.app.v2 "${APP_PATH}"
else
