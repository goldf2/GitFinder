...
const ELECTRON_FUSE_INDEX = Object.freeze({
  RunAsNode: 0,
...
const FORBIDDEN_ASAR_ROOTS = new Set([
...
function validateElectronFuses(states = {}) {
  const mismatches = Object.entries(REQUIRED_FUSE_CONFIG)
    .map(([configName, expected]) => {
      const artifactName = Object.keys(ELECTRON_FUSE_INDEX)
        .find((name) => name.charAt(0).toLowerCase() + name.slice(1) === configName);
      return artifactName && states[artifactName] === expected ? null : artifactName || configName;
    })
    .filter(Boolean);
  return {
    issues: mismatches.length
      ? [issue('fuses.artifact', `Electron Fuses 与发布基线不一致：${mismatches.join(', ')}`)]
      : [],
  };
}

function validatePackagedContents(entries = []) {
  const forbidden = [...new Set(entries
    .map((entry) => String(entry || '').replace(/^\/+/, '').split('/')[0])
    .filter((root) => FORBIDDEN_ASAR_ROOTS.has(root)))];
  return {
    issues: forbidden.length
      ? [issue('app.asar-contents', `app.asar 夹带了非运行时文件：${forbidden.join(', ')}`)]
      : [],
  };
}

function readElectronFuses(appPath) {
  ...
  if (sentinelIndex < 0) throw new Error('Electron 二进制文件中缺少 Fuse Wire。');
  const wireStart = sentinelIndex + sentinel.length;
  const version = binary[wireStart];
  const wireLength = binary[wireStart + 1];
  if (version !== 1) throw new Error(`不支持的 Electron Fuse 版本：${version}`);

  const states = {};
  for (const [name, index] of Object.entries(ELECTRON_FUSE_INDEX)) {
    if (index >= wireLength) {
      states[name] = undefined;
      continue;
    }
    const value = binary[wireStart + 2 + index];
    states[name] = value === 0x31 ? true : (value === 0x30 ? false : 'removed');
  }
  return states;
}
...
function verifyArtifact(projectRoot, options, source) {
  ...
  try {
    ...
    const packagedEntries = asar.listPackage(asarPath, { isPack: true });
    packagedEntryCount = packagedEntries.length;
    issues.push(...validatePackagedContents(packagedEntries).issues);

    const rawHeader = asar.getRawHeader(asarPath);
    const expectedIntegrity = crypto.createHash('sha256').update(rawHeader.headerString).digest('hex');
    const embeddedIntegrity = readPlistValue(
      plistPath,
      'ElectronAsarIntegrity:Resources/app.asar:hash',
    );
    const embeddedAlgorithm = readPlistValue(
      plistPath,
      'ElectronAsarIntegrity:Resources/app.asar:algorithm',
    );
    if (embeddedAlgorithm !== 'SHA256' || embeddedIntegrity !== expectedIntegrity) {
      issues.push(issue('app.asar-integrity', 'Info.plist 中的 app.asar 完整性摘要缺失或不一致。'));
    }
  } catch {
  ...
  try {
    electronFuses = readElectronFuses(appPath);
    issues.push(...validateElectronFuses(electronFuses).issues);
  } catch (error) {
    issues.push(issue('fuses.read', `无法验证 Electron Fuses：${error.message}`));
  }
  ...
  if (!zipEntries.ok
    || !zipEntries.output.split('\n').some((entry) => entry === `${PRODUCT_NAME}.app/Contents/MacOS/${PRODUCT_NAME}`)
    || zipEntries.output.split('\n').some((entry) => entry && !entry.startsWith(`${PRODUCT_NAME}.app/`))) {
    issues.push(issue('artifact.zip-layout', `macOS ZIP 必须只包含根目录 ${PRODUCT_NAME}.app。`));
  }
